...
bool FloodView::start()
{
  ...
  float tx = -((m_dem->cols / 2.0 + m_dem->rows / 2.0) / 2.0) * m_dem->gridsize;
  float ty = -tx;
...
void FloodView::initCamera(void)
{
  ...
  glLoadIdentity();
  gluPerspective(45.0, 1.0, 0.1, thiz->m_dem->gridsize * thiz->m_dem->gridsize * 1000.0);
  glTranslated(0, 0, -5);
...
void FloodView::draw()
{
  ...
  glScaled(m_dem->gridsize, -m_dem->gridsize, m_zscale);
...
void mouseClickCallback(int button, int state, int x, int y) 
{
  ...
  switch (button) {
    ...
    case 3: // scroll up
      ...
      if (state == GLUT_DOWN) {
        thiz->m_translation.z += (thiz->m_dem->gridsize * thiz->m_dem->gridsize) / 2;
        glutPostRedisplay();
      }
      break;
    case 4: // scroll down
...
void mouseMotionCallback(int x, int y)
{
  ...
  if (! modifiers && ! thiz->m_rotateAction) {
    thiz->m_translation.x += ((double)(x - thiz->m_mouseCoordinate.x) * thiz->m_dem->gridsize);
    thiz->m_translation.y -= ((double)(y - thiz->m_mouseCoordinate.y) * thiz->m_dem->gridsize);
...
bool FloodView::run()
{
  ...
  rotated_dem->gridsize = m_dem->gridsize;
